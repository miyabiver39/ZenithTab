...
- **追加的な変更**(新しい任意フィールド)はスキーマ版を上げない。読み込み時のハイドレーション( = レジストリの  で欠けたキーだけ補完、 /  の浅いマージ)が面倒を見る。
- 既定値の補完で**ユーザーの見え方が変わる**場合(例: 旧 RSS の  に  の既定を足すと検索がトップニュースに化ける)は、 の  で旧データから正しい値を導く。
- **破壊的変更**(キー名の変更、値の構造変更、キーの統合/分割)は  で  を +1 し、 に前進・冪等なステップを1つ追加する。直前のスナップショット()と1版ずつのコミットは基盤側が行う。
- 旧キーの削除は**2リリースに分ける**(N: 新キーに書き旧キーも読む → N+1: 旧キー削除)。
- マイグレーションは新しいタブの  先頭でだけ走る(Service Worker では走らせない)。エクスポート JSON のインポートも同じ関数を通る。
- 新しいバージョンを出したら  を1つ追加し、 の対象に加える。
